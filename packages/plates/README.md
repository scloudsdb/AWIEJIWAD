...
```bash
pnpm --filter @AI3DLabs/plates build-meshes
```
...
```ts
import { createBuildPlate } from '@AI3DLabs/plates/three';
import { mountPlatePicker, loadPlateChoice } from '@AI3DLabs/plates';
...
generator template (via `@AI3DLabs/viewer`, which builds the plate in for you).
