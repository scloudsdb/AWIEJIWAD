...
`pnpm --filter @AI3DLabs/fonts fetch-fonts` — don't hand-edit them.

## Two ways a font is used, and they are not the same

- **Preview** — the HTML font grid renders in `VL-<id>` from `fonts.css`. Import
  it once in your app's stylesheet: `@import '@AI3DLabs/fonts/fonts.css';`
...
2. `pnpm --filter @AI3DLabs/fonts fetch-fonts`
