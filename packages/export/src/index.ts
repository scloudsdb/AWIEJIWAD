...
export interface ExportMeta {
  ...
  generator: string;
  /** Human application name, e.g. "A&I 3D Labs Magnet Generator". */
  application?: string;
 ...
 *  Designer/Application metadata still name A&I 3D Labs. */
...
export const PROVENANCE_FILE = 'Metadata/AI3DLabs_labs.txt';
...
export interface ProvenanceMeta {
  ...
  buildId?: string;
  /** Human name of the app, defaults to "A&I 3D Labs <title>". */
  application?: string;
}
 ...
 * all: no Designer, no Copyright, no LicenseTerms, no AI3DLabs_labs.txt. Every export path in
...
export function provenance(meta: ProvenanceMeta): { metadata: string; text: string } {
  ...
  const metadata =
    ...
    `<metadata name="LicenseTerms">${esc(
      `CC BY-NC-ND 4.0. Personal use only. Commercial use requires a license: ${BRAND.urls.commercialLicense}`,
    )}</metadata>` +
