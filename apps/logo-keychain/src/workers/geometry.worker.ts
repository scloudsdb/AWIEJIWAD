...
import { buildLogoKeychain } from '../geometry/buildLogoKeychain';
  ...
  try {
    ...
    if (msg.type === 'init') {
      ...
      /* The clear column an MX switch needs, in millimetres, measured off the socket asset
         the way `buildLogoKeychain` measures it (`switchClear = socketDim + 3`). Reported rather
    ...
    if (msg.type === 'buildLogoKeychain') {
      if (!socket || !stem) throw new Error('Assets not initialized');
      const { parts, warnings } = buildLogoKeychain(
        wasm,
        msg.regions,
        msg.outline,
        msg.params,
