...
export interface UiCallbacks {
  ...
  onRingStyle(style: 'loop' | 'corner'): void;
  onHoleDia(mm: number): void;
  onRingThickness(mm: number): void;
  onRingPos(mm: number, axis: 'x' | 'y'): void;
  onRingAngle(deg: number): void;
...
export function createUi(
  ...
  const flushToggle = toggleSwitch({
    label: 'Flush Logo',
    help: 'Makes the logo flush with the plate rather than sticking out.',
    checked: initial.flushLogo,
    onChange: (v) => cb.onFlushLogo(v),
  });
  $('flushMount').append(flushToggle);

  const ringStyleRow = segmentedControl({
    label: 'Ring style',
    value: initial.ringStyle,
    options: [{ value: 'loop', label: 'Loop' }, { value: 'corner', label: 'Corner' }],
    onChange: (v) => cb.onRingStyle(v),
  });
  $('ringStyleMount').append(ringStyleRow);
  ...
  $('holeDiaMount').append(holeDiaRow);
  ...
  $('ringThickMount').append(ringThickRow);
  ...
  posContainer.className = 'vl-field-group';
  posContainer.style.display = 'flex';
  posContainer.style.gap = '8px';
  posContainer.appendChild(ringPosRowX);
  posContainer.appendChild(ringPosRowY);
  $('ringPosMount').append(posContainer);
  ...
  $('ringAngleMount').append(ringAngleRow);
  ...
  function update(state: UiState) {
    ...
    flushToggle.setValue(state.flushLogo);
    ringStyleRow.setValue(state.ringStyle);
    holeDiaRow.setValue(state.holeDia);
    ringThickRow.setValue(state.ringThickness);
    ringPosRowX.setValue(state.ringPosX);
    ringPosRowY.setValue(state.ringPosY);
    ringAngleRow.setValue(state.ringAngle ?? 180);
    ...
    for (const mount of ['topthickMount', 'imgdepthMount']) {
