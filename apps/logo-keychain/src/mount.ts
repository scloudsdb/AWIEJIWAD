...
import { mountPlatePicker } from '@AI3DLabs/plates';
...
import type { DesktopHost } from '@AI3DLabs/ui-kit';
import { closeAllDialogs, dialog } from '@AI3DLabs/ui-kit';
...
export function mount(container: HTMLElement, host?: DesktopHost): () => void {
  ...
  // Mount the unified AI3DLabs topbar — except in the MakerWorld build, where the host provides
  ...
  if (oldTopbar) {
    if (MAKERLAB || isDesktop()) {
      ...
      oldTopbar.replaceWith(topbarLinks({
        appTitle: 'Clicker Generator',
        donateUrl: BRAND.urls.donate,
        themeToggle: true,
      }));
    }
  }
  ...
  const ui: ReturnType<typeof createUi> = createUi(sidebarLeft, sidebarRight, statusEl, {
    ...
    onFlushLogo: (on) => {
      ...
      onRingStyle: (style) => {
        store.set({ ringStyle: style });
        debouncedRebuild();
      },
      onHoleDia: (mm) => {
        store.set({ holeDia: mm });
        debouncedRebuild();
      },
      onRingThickness: (mm) => {
        store.set({ ringThickness: mm });
        debouncedRebuild();
      },
      onRingPos: (mm, axis) => {
        store.set(axis === 'x' ? { ringPosX: mm } : { ringPosY: mm });
        debouncedRebuild();
      },
      onRingAngle: (deg) => {
        store.set({ ringAngle: deg });
        debouncedRebuild();
      },
  ...
  const HISTORY_FIELDS = [
    ...
    'imageDepth', 'flushLogo', 'ringStyle', 'holeDia', 'ringThickness', 'ringPosX', 'ringPosY', 'ringAngle', 'fixedSize', 'designScale', 'shapeSides', 'shapeCornerPct',
  ...
  // Load A&I 3D Labs logo sample on startup
  ...
  (async function loadSvgSamples() {
    for (const sample of SVG_SAMPLES) {
      try {
        const svgText = await fetch(sample.src).then((r) => {
  ...
  async function loadDefaultClicker() {
    try {
      ...
      // If it's a hybrid default file (contains both prebuilt parts and settings)
      let partsToLoad = serializedParts;
      let projectSettings = null;
      
      if (!Array.isArray(serializedParts) && serializedParts.parts) {
        partsToLoad = serializedParts.parts;
        projectSettings = serializedParts;
      } else if (!Array.isArray(serializedParts) && (serializedParts.version || serializedParts.settings)) {
        // Pure project file (no parts, needs trace)
        defaultClickerLoaded = true;
        isInitialLoad = false;
        await applyProject(serializedParts);
        resetHistory();
        return;
      }
      
      const parts: ClickerPart[] = partsToLoad.map((p: any) => ({
        kind: p.kind,
        group: p.group,
        colorRgb: p.colorRgb,
        name: p.name,
      ...
      if (projectSettings) {
        // Apply settings silently without triggering a rebuild so the UI matches the parts
        const set = projectSettings.settings ?? {};
        ...
        if (set.importMode === 'image' && projectSettings.image) {
          // Fire-and-forget image decode so it's ready if they trigger a rebuild
          dataUrlToImage(projectSettings.image).then(img => {
            originalImage = img;
          }).catch(console.error);
        }
        ...
        store.set({
          ...
          ringStyle: set.ringStyle ?? 'loop',
          holeDia: set.holeDia ?? 5.6,
          ringThickness: set.ringThickness ?? 2.5,
          ringPosX: set.ringPosX ?? 0,
          ringPosY: set.ringPosY ?? 0,
          ringAngle: set.ringAngle ?? 180,
          fixedSize: set.fixedSize ?? null,
          designScale: set.designScale ?? 1,
          shapeSides: set.shapeSides ?? 6,
          ...
          switches: Array.isArray(set.switches) && set.switches.length ? set.switches : [{ x: 0, y: 0, rotation: 0 }],
          activeSwitchIndex: 0,
          keychain: set.keychain && typeof set.keychain === 'object' ? { offsetMm: 0, ...set.keychain } : { enabled: false, style: 'loop', angleDeg: 90, holeDiameterMm: 5.2, offsetMm: 0 },
          ...
          bodyColorRgb: set.bodyColorRgb ?? [240, 240, 240],
          ...
          baseColorOverride: set.baseColorOverride ?? null
        });
        
        if (Array.isArray(projectSettings.palette)) {
           store.set({ palette: projectSettings.palette });
        }
      }
      store.set({
        building: false,
  ...
  function reprocess(isLazyTrace = false) {
    ...
    if (store.get().importMode !== 'blocks' && !isLazyTrace) store.set({ baseColorOverride: null });
    const s = store.get();
  ...
  function buildParamsFor(s: UiState): BuildParams {
    ...
    return {
      ...
      bodyColorRgb: s.bodyColorRgb ?? ([240, 240, 240] as RGB),
  ...
  function rebuild(quiet = false) {
    ...
    if (!regionSet && (store.get().importMode !== 'image' || originalImage)) {
      reprocess(true);
      return;
    }
  ...
  // --- ADDED BY AI FOR EXPORTING DEFAULT CLICKER ---
  (window as any).exportDefaultClicker = () => {
    const serializedParts = latestParts.map((p: any) => ({
      kind: p.kind,
      group: p.group,
      colorRgb: p.colorRgb,
      name: p.name,
      numProp: p.numProp,
      vertProperties: Array.from(p.vertProperties),
      triVerts: Array.from(p.triVerts)
    }));
    
    // Create hybrid format that contains BOTH project settings AND the prebuilt geometry
    const hybridExport = {
      ...buildProject(),
      parts: serializedParts,
    };
    
    const blob = new Blob([JSON.stringify(hybridExport)], {type: "application/json"});
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "default-clicker.json";
    a.click();
    console.log("Exported default-clicker.json successfully (Hybrid format)!");
  };
  // --------------------------------------------------
