...
import '@AI3DLabs/fonts/fonts.css';
import './style.css';
...
// @AI3DLabs/fonts so every generator that puts type on a model shares one set.
import {
  FONTS,
  type FontChoice,
  getFont,
...
export function mount(container: HTMLElement, host?: DesktopHost): () => void {
  ...
  async function handleExport(formatId: string) {
    if (formatId === '3mf') {
      ...
      const safeName = state.name.trim().replace(/[^a-z0-9]+/gi, '-');
      const fn = `ai3dlabs-${safeName || 'Name'}.3mf`;
      if (host) {
        ...
        // Removed openLicenseModal since we want direct download and Patreon is in the top bar.
      }
