...
export function createUi(
  ...
  const headerEl = generatorHeader({
    title: 'ID Card Holder Generator',
    description: 'Generate printable 3D model of an ID Card / e-Money Holder from an image',
  });
  ...
  const qualityEl = MAKERLAB ? null : qualityCallout({
    html: `For the best print quality, we recommend using a <strong>0.2mm layer height</strong>, <strong>2 walls</strong>, and <strong>15% infill</strong>.`,
  ...
  leftScroll.innerHTML = `
    ...
    <!-- Card design section (replaces Base Style for cardless) -->
    <div class="section" id="cardInfoSection">
      <span class="label">Card design ${tip('Upload your image or SVG — it will be embossed on the front of the card holder.')}</span>
      <div class="prow-stacked">
        <div id="logoScaleMount"></div>
      </div>
      <p class="vl-hint" style="margin-top:8px;">Case fits: 85.6 × 54 mm cards &mdash; Flazz, e-money, debit, ATM</p>
    </div>

    <!-- Hidden: original mounts required by JS but not displayed in cardless -->
    <div style="display:none;">
      <div id="showSwitchMount"></div>
      <div id="blocksSection"></div>
      <div id="textSection">
        <div id="textScaleMount"></div>
        <div id="textBoldMount"></div>
        <div id="letterSpacingMount"></div>
        <div id="lineSpacingMount"></div>
      </div>
      <div id="baseStyleSection">
        ...
        <div id="fixedSizeFields">
          <div id="fixedWMount"></div>
          <div id="fixedHMount"></div>
          <div id="fixedSizePresetMount"></div>
        </div>
        <div id="imageNudgeField">
          <div id="imageNudgePadMount"></div>
        </div>
      </div>
      <div id="sectionSwitch">
        <div id="switchCountMount"></div>
        <div id="switchChipsMount"></div>
        <div id="switchPadMount"></div>
        <button id="switchResetAll" type="button"></button>
      </div>
      <!-- Block mode mounts -->
      ...
      <div id="blocksTextField"></div>
      <div id="blocksChainField"></div>
      <div id="capProudMount"></div>
      <div id="hollowMount"></div>
      <div id="stemFitMount"></div>
      <div id="socketFitMount"></div>
    ...
    <div id="geometrySettingsContainer">
      ...
      <details class="vl-section vl-section--collapsible" id="sectionColors">
        ...
        <div class="vl-section__body">
          <p class="vl-hint">Select a color bucket on the right to recolor, or left-click parts on the 3D model.</p>
          <div class="field" id="colorCountField" style="display:none;">
            <div id="ccountMount"></div>
          </div>
          <div class="prow-stacked" id="smoothingField" style="display:none;">
      ...
      <details class="vl-section vl-section--collapsible" id="sectionKeychain">
        <summary>Keychain loop</summary>
        <div class="vl-section__body">
          <p class="vl-hint" style="margin-bottom:8px;">Add a loop for a split ring.</p>
          <div class="prow-stacked"><div id="keychainMount"></div></div>
          <div id="keychainOpts">
            <div id="keychainAngleRow"><div id="keychainAngleMount"></div></div>
            <div id="keychainOffsetRow"><div id="keychainOffsetMount"></div></div>
            <div id="keychainFreeResetMount"></div>
            <div class="prow-stacked"><div id="keychainSizeMount"></div></div>
            <div id="keychainEndField" style="display:none;"><div id="keychainEndMount"></div></div>
            <div id="keychainSlideRow" style="display:none;"><div id="keychainSlideMount"></div></div>
            <div id="keychainBlockResetMount" style="display:none;"></div>
          </div>
        </div>
      </details>
  ...
  if (MAKERLAB) {
    // MakerWorld review feedback (2026-07-27): the A&I 3D Labs intro block is the most
    ...
    <div class="section" id="importSourceSection">
      ...
      <div id="iconPanel" class="mode-panel" hidden>
        <div id="iconSearchWrap">
          <input id="iconSearch" type="search" placeholder="Search clipboard icons..." autocomplete="off" spellcheck="false" />
      ...
      <div id="letterPanel" class="mode-panel" hidden>
        ...
        <div class="field">
          ...
          <label class="button secondary" style="display: flex; justify-content: center; align-items: center; gap: 8px; margin-top: 8px; font-weight: 500;">
            ...
            Import font
            <input id="fontUpload" type="file" accept=".ttf,.otf,.json,font/ttf,font/otf,application/json" style="display: none;" />
          </label>
        </div>
  ...
  // Keep blocks-mode helpers for potential future use

  /** Edit one block: retype its letter, swap in a symbol, blank the cell, or delete it. */
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  function openSlotEditorNoop(_index: number, _anchor: HTMLElement) {
  ...
  const topthickRow = sliderRow({
    label: 'Lid thickness', help: 'Thickness of the front lid (the face with the embossed logo), in mm.',
    min: 1.5, max: 5, step: 0.1, value: initial.topThickness,
  ...
  const imgdepthRow = sliderRow({
    label: 'Logo emboss depth', help: 'How far the logo is raised proud of the lid surface, in mm.',
    min: 0.2, max: 2, step: 0.1, value: initial.imageDepth,
    format: (v) => `${v.toFixed(1)} mm`,
  ...
  $('updatesMount').append(changelogButton({ entries: CHANGELOG, title: 'Cardless updates' }));

  // Logo Scale slider (in Card Design section)
  const logoScaleRow = sliderRow({
    label: 'Logo scale',
    help: 'How large the embossed logo is on the front of the case. 100% = fill the case face.',
    min: 20, max: 100, step: 5, value: 80,
    format: (v) => `${v.toFixed(0)} %`,
    onInput: (v) => cb.onDesignScale(v / 100),
  });
  $('logoScaleMount').append(logoScaleRow);
  ...
  function showColorPopoverAt(
    ...
    // Prevent browser context menu if user right-clicks on the popover itself
    popover.addEventListener('contextmenu', (e) => e.preventDefault());
  ...
  function update(state: UiState) {
    ...
    keychainSizeRow.setValue(state.keychain.holeDiameterMm);
    ...
    // Keychain UI state
    keychainToggle.setValue(state.keychain.enabled);
    const keychainOpts = document.getElementById('keychainOpts');
    if (keychainOpts) {
      keychainOpts.style.display = state.keychain.enabled ? 'block' : 'none';
      keychainAngleRow.setValue(state.keychain.angleDeg);
      keychainOffsetRowCtl.setValue(state.keychain.offsetMm);
    }
