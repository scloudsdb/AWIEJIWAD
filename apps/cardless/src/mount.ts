...
export function mount(container: HTMLElement, host?: DesktopHost): () => void {
  ...
  if (oldTopbar) {
    if (MAKERLAB || isDesktop()) {
      ...
      oldTopbar.replaceWith(topbarLinks({
        appTitle: 'ID Card Holder Generator',
  ...
  const ui: ReturnType<typeof createUi> = createUi(sidebarLeft, sidebarRight, statusEl, {
    ...
    onExport: async () => {
      ...
      if (MAKERLAB && mlReady() && mlCan('export')) {
        ...
        try {
          const { obj, mtl } = buildObjMtl(latestParts, 'cardless.mtl');
          ...
          const result = await sdkExport({
            artifacts: [
              {
                fileName: 'cardless.obj',
                format: 'obj',
                buffer: objToArrayBuffer(obj),
                mtl,
                coverImage,
                description: 'Multi-color ID card holder, made with the ID Card Holder Generator.',
              },
            ],
          });
          if (result.success) {
        ...
        try {
          const { indexed } = await host.exportToLibrary(
            { name: 'ai3dlabs-cardless.3mf', bytes: buildThreeMF(latestParts) },
            { designer: 'ID Card Holder Generator' },
          );
          store.set({ status: indexed ? 'Exported to your library ✅' : 'Exported as ai3dlabs-cardless.3mf ✅' });
        } catch (err) {
          store.set({ status: 'Export failed: ' + String(err) });
        }
      } else {
        // Standalone / public path: direct browser download.
        downloadThreeMF(latestParts, 'ai3dlabs-cardless.3mf');
      }
    },
    onRenderPng: async () => {
  ...
  viewer.onPartPick((index, ev: any) => {
    // Viewer might pass (index, ev) or (index, x, y, shift) depending on the package version
    let clientX, clientY, shiftKey, isLeftClick = false;
    
    if (typeof ev === 'object' && ev !== null && 'clientX' in ev) {
      clientX = ev.clientX;
      clientY = ev.clientY;
      shiftKey = ev.shiftKey;
      // Only block pure left-clicks without modifiers
      if (ev.type === 'pointerup' && ev.button === 0 && !ev.ctrlKey && !ev.metaKey) {
        isLeftClick = true;
      }
    } else {
      // Fallback for older viewer signatures
      clientX = typeof ev === 'number' ? ev : window.innerWidth / 2;
      clientY = typeof arguments[2] === 'number' ? arguments[2] : window.innerHeight / 2;
      shiftKey = arguments[3];
      // If we can't tell, assume it's valid so we don't break the UI
      isLeftClick = false;
    }
    ...
    if (s.editMode === 'color') {
      // User requested: "jangan ada keluar pilih warna cukup klik kanan"
      // So we ignore standard left-clicks, but allow right-clicks, contextmenus, etc.
  ...
  function buildOne(
    ...
    return new Promise((resolve) => {
      ...
      worker.postMessage({ type: 'buildCardHolder', regions, outline, params, requestId });
    });
  }
  ...
  worker.onmessage = (e: MessageEvent<GeometryResponse>) => {
    ...
    switch (msg.type) {
      ...
      case 'initDone':
        ...
        // Build the base case immediately on startup (no image needed)
        reprocess();
        break;
      case 'parts': {
  ...
  async function initAssets() {
    try {
      ...
      store.set({ status: 'Failed to init geometry worker: ' + String(err) });
      isInitialLoad = false;
    }
  }
  ...
  function rebuild(quiet = false) {
    ...
    // In Cardless, we build the case even if there is no image (regionSet is null)
    const activeRegionSet = regionSet || { regions: [], outline: [] };
    ...
    activeRegionSet.regions.forEach((r, i) => {
    ...
    if (isBlocks) {
      ...
      worker.postMessage({ type: 'buildCardHolder', regions, outline: activeRegionSet.outline, params });
    }
  }
