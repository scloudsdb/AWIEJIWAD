...
} from '@AI3DLabs/export';
import { plateSize, type PlateChoice } from '@AI3DLabs/plates';
...
const COMMERCIAL_URL = BRAND.urls.commercialLicense;
// Custom metadata namespace (need not resolve; identifies our provenance keys).
const VL_NS = 'https://ai3dlabs.com/3mf/2026';
...
export function buildThreeMF(parts: ClickerPart[], opts: ThreeMFOptions = {}): Uint8Array {
  ...
  const metadata =
    BBL_VERSION_META +
    `<metadata name="Title">ID Card Holder</metadata>` +
    `<metadata name="Designer">${esc('A&I 3D Labs')}</metadata>` +
    `<metadata name="Application">${BBL_APPLICATION}</metadata>` +
    `<metadata name="vl:application">${esc('A&I 3D Labs ID Card Holder Generator')}</metadata>` +
    `<metadata name="CreationDate">${creationDate}</metadata>` +
    `<metadata name="Copyright">${esc('© A&I 3D Labs. Generated by the A&I 3D Labs ID Card Holder Generator.')}</metadata>` +
    ...
    `<metadata name="vl:generator">cardless-generator</metadata>` +
  ...
  const provenance = [
    'A&I 3D Labs - ID Card Holder Generator',
    '',
    'This 3MF was generated by the A&I 3D Labs ID Card Holder Generator.',
    `Build: ${buildId}`,
  ...
  return zipSync(
    {
      ...
      'Metadata/AI3DLabs_labs.txt': strToU8(provenance),
    },
    { level: 6 },
  );
}

export function downloadThreeMF(parts: ClickerPart[], fileName = 'cardless.3mf', opts: ThreeMFOptions = {}) {
